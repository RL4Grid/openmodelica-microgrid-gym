...
n_MC = 1
v_DC = 460 / 2  # DC-link voltage / V; will be set as model parameter in the FMU
...
class Reward:
    ...
    def rew_fun(self, cols: List[str], data: np.ndarray) -> float:
        ...
        error = (np.sum((np.abs((vsp_abc_master - vabc_master)) / vLimit) ** 0.5, axis=0) \
                 + -np.sum(mu * np.log(1 - np.maximum(np.abs(vabc_master) - vNominal, 0) / (vLimit - vNominal)),
                           axis=0)) \
                / max_episode_steps
...
if __name__ == '__main__':
    ...
    ctrl = MultiPhaseDQ0PIPIController(voltage_dqp_iparams, current_dqp_iparams, delta_t, droop_param, qdroop_param,
                                       observer=[Lueneberger(*params) for params in
                                                 repeat((A, B, C, L, delta_t, v_DC), 3)], undersampling=1,
                                       name='master')
    ...
    if include_simulate:
        class PlotManager:
            ...
            def __init__(self, used_agent: SafeOptAgent, used_r_load: Load, used_r_filt: Load, used_l_filt: Load,
                         used_c_filt: Load, used_i_noise: Noise):
        ...
        env = gym.make('openmodelica_microgrid_gym:ModelicaEnv_test-v1',
                       ...
                       model_params={'lc.resistor1.R': R_filt,  # partial(r_filt.load_step, n=0),
                                     ...
                                     'lc.resistor4.R': 0.0000001,  # partial(r_filt.load_step, n=0),
                                     'lc.resistor5.R': 0.0000001,  # partial(r_filt.load_step, n=1),
                                     'lc.resistor6.R': 0.0000001,  # partial(r_filt.load_step, n=2),
